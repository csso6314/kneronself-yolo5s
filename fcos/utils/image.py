...
def preprocess_image(x, mode='kneron'):
    """
...
#     h,w,c = img.shape
#     n_h, n_w = make_divisable(h), make_divisable(w)
#     new_img = np.zeros((n_h, n_w, c))
#     new_img[:h,:w,:] = img
...
def resize_image(img, min_side=800, max_side=1333):
    ...
    h,w,c = img.shape
    n_h, n_w = make_divisable(h), make_divisable(w)
    new_img = np.zeros((min_side, max_side, c))
    new_img[:h,:w,:] = img
